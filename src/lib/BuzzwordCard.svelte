...
<div class="card bg-surface-200 max-w-fill" data-testid="buzzword-card" id={buzzword.id}>
    <header class="card-header flex gap-x-10">
        {#if buzzword.author}
            <a href="{base}/about/people/{buzzword.author}"><img class="inline-block h-16 w-16 rounded-full border-solid border-2 border-primary-500" src="{base}/thumbnails/{people[buzzword.author]['img']}" alt=""/></a>
        {/if}
        
        <div class="flex flex-col gap-2">
        {#if buzzword.author}
            <a href="{base}/about/people/{buzzword.author}" class="byline font-extralight text-xs" data-testid="buzzword-byline">{getAuthorName(buzzword.author)}</a>
        {/if}
