...
<div class="card bg-surface-200 max-w-fill" data-testid="buzzword-card" id={buzzword.id}>
    <header class="card-header flex gap-x-10">
        {#if buzzword.author}
            <a href="{base}/about/people/{buzzword.author}"><img class="inline-block h-16 w-16 rounded-full border-solid border-2 border-primary-500" src="{base}{people[buzzword.author]['img']}" alt=""/></a>
        {/if}
