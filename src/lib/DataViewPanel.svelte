...
{#if loaded}
    {#if error == 0}
            <h2>{datasets[0].desc.metadata.title}</h2>

                {#if $activeView === 'data'}
                    <GroupSelector groups={getGroups('Treatment group', entry.data)} name = {'Treatment group'} bind:selected = {selected}/>
                    <RawDataTable tableObject={{data: entry.data, columns: entry.columns}} selected = {selected}/>
                {:else if $activeView === 'summary'}
                    <GroupSelector groups={getGroups('Treatment group', entry.data)} name = {'Treatment group'} bind:selected = {selected}/>
                    <RawDataTable tableObject = {{data: entry.summaryData, columns: entry.summaryColumns}} selected = {selected}/>
                {:else if $activeView === 'visualisation'}
                    <GroupSelector groups={getGroups('Treatment group', entry.data)} name = {'Treatment group'} bind:selected = {selected}/>
                    <DataViz dataObject = {{data: entry.summaryData, labels: entry.summaryColumns}} bind:selected = {selected} name = {entry.desc.metadata.title} rawData = {entry.data}/>
