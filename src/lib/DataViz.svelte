<script>
        ...
        try {
            if (loaded) {
                ...
                if (newSelection === "All") {
                    ...
                    filteredData = dataObject.data.filter(
                        (entry) => entry["Treatment group"] === selected
                    );
                }
    ...
    let dpMouseleave = function (event, d) {
        tooltip = d3.select(`#tooltip-${makeHtmlId(name)}`);
        tooltip.html("").classed("opacity-0", true);
        d3.select(this)
    ...
    let labelMouseover = function (event, d) {
        ...
        d3.select(this).classed("font-bold", true).classed("font-light", false);
    ...
    let labelMouseleave = function (event, d) {
        ...
        d3.select(this).classed("font-bold", false);
        ...
        try {
            ...
            dataPoint
                ...
                .attr("fill", function (d) {
                    return colour(d["Treatment group"]);
                })
                .classed("opacity-75", true)
            ...
            // draw error lines
            let errorLines = dataPoint
                .append("g")
                .attr("class", "error-data data");
            ...
            errorLines
                ...
                .attr("class", function (d) {
                    return makeHtmlId(d["Treatment group"]);
                })
        ...
        try {
            ...
            if (svg.empty()) {
                throw new ReferenceError(
                    `Could not create the svg, div with id ${makeHtmlId(
                        name
                    )} does not exist on mount.`
                );
            }
    ...
    <div id="line-graph-{makeHtmlId(name)}" data-testid="line-graph">
        ...
        {#if errorCode > 0}
            <p class="error-message">
                Error: No data passed (Error code: {errorCode})
            </p>
