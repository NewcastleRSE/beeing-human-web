...
test.describe('Buzzwords container tests', () => {
    test.beforeEach('Open start URL', async ({ page }, testInfo) => {
        console.log(`Running ${testInfo.title}`);
        await page.goto('/connections/buzzwords-feed');
    });
    ...
    test('Page should have one search button for the search bar', async ({ page }) => {
        await expect(page).toHaveURL('/connections/buzzwords-feed');
        const resetButton = page.getByTestId('search-button', { exact: true });
...
test.describe('Page has correct contents tests - buzzwords', () => {
    let buzzwords;
    let listTags;
    let listAuthors;
    test.beforeEach('Open start URL and load data from files', async ({ page, request }, testInfo) => {
        console.log(`Running ${testInfo.title}`);

        let response = await request.get('/api/buzzwords');
        const buzzwordData = await response.json();

        buzzwords = buzzwordData.buzzwords;
        // turns date string into date object
        for (const buzz of buzzwordData.buzzwords) {
            buzz.date = new Date(buzz.date);
        }
        listTags = buzzwordData.buzzwordTags;
        listAuthors = buzzwordData.buzzwordAuthors;
        await page.goto('/connections/buzzwords-feed');
    ...
    test('Each buzzword that contains an image has the image displayed', async ({ page }) => {
        ...
        for (let buzz of buzzImage) {
            ...
            for (let img of imgs) {
                ...
                // gets alt text without md syntax and removing the `{base}` from the start of the url
                const altText = img.groups.alt.slice(2, -1);
                const imgSrc = img.groups.filename.substring(6);
...
test.describe('Page user interactions tests - buzzwords', () => {
    let buzzwords;
    let listTags;
    let listAuthors;
    test.beforeEach('Open start URL and load data from files', async ({ page, request }, testInfo) => {
    ...
    test.describe('Test interaction with tags filter', () => {
        ...
        test('Clicking the same button twice should reactivate all tag buttons', async ({page}) => {
            await expect(page).toHaveURL('/connections/buzzwords-feed');
